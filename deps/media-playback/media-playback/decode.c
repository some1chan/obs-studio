...
static int get_stream(mp_media_t *m, struct mp_decode *d, enum AVMediaType type,
		      int track)
{
	if (d->audio) {
		if (track == -1) // Disable audio
			return -1;
		int audio_track_c = 0;
		for (unsigned int i = 0; i < m->fmt->nb_streams; i++) {
			if (m->fmt->streams[i]->codecpar->codec_type ==
			    AVMEDIA_TYPE_AUDIO) {
				audio_track_c += 1;
				if (audio_track_c == track)
					return i;
			}
		}
		if (track > 0)
