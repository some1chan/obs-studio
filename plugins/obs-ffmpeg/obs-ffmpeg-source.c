					...
					obs_data_t *settings)
{
	...
	obs_property_t *audio_track = obs_properties_get(props, "audio_track");
	int current_audio_track_number =
	...
	for (int i = 0; i < (int)fmt_ctx->nb_streams;
		...
		if (codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
			...
			char buf_str[20];
			snprintf(buf_str, 20, "Audio Track %d",
				 nb_audio_streams);
			     ...
			     const char *input_format)
{
	FF_BLOG(LOG_INFO,
		...
		s->close_when_inactive ? "yes" : "no", s->audio_track,
		s->ffmpeg_options);
}
...
static void ffmpeg_source_update(void *data, obs_data_t *settings)
{
	...
	s->audio_track = obs_data_get_int(settings, "audio_track");
